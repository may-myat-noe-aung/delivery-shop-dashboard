...
export default function NotificationFetcher() {
  ...
  const fetchData = async () => {
    try {
      ...
      const prevOrderCount = parseInt(
        localStorage.getItem(orderKey) || "0"
      );
      ...
      if (
        currentOrderCount > prevOrderCount &&
        orders.length > 0
      ) {
        const latestOrder = orders[0];
        ...
        addNotification({
          ...
          message: `New order received by ${
            latestOrder.name || "Unknown"
          }
Order ID - ${latestOrder.id}`,
          orderId: latestOrder.id,
          name: latestOrder.name,
...
Customer: ${
            latestCompleted.order.name || "Unknown"
          }
Delivery: ${
            latestCompleted?.deliveryman?.name || "N/A"
          }
Total: ${latestCompleted.order.grand_total?.toLocaleString()} MMK`,
      ...
      localStorage.setItem(
        reportKey,
        currentCompletedCount.toString()
      );
    } catch (err) {
      console.error("Notification error:", err);
    }
